...
реальной командой проекта; frontend failure вызывает полный откат. Линковка и автоматическая сборка
затронутой цели пока не запускаются. Для автоматической обработки любой результат можно вывести в
JSON:
